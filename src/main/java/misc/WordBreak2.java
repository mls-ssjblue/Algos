...
//                        Arrays.asList("apple", "pen", "applepen", "pine", "pineapple")));
    ...
    private List<String> wordBreak(String s, List<String> wordDict) {
        ...
        return wordBreakUtil(s,wordDict,  results);
    }


    private List<String> wordBreakUtil(String s, List<String> wordDict,  List<String> results) {
        ...
        for(int i = 0; i < s.length(); i ++){
            ...
            if(wordDict.contains(sub)){
                List<String> res = wordBreakUtil(s.substring(i+1), wordDict, results);
                for (String re : res) {
                ...
                if(newRes.size() > 0)possible = 1;
            }
        }
        return possible == 1? newRes: Collections.emptyList();
    }
}
